# PDA-INNO 프로젝트 규칙

## 해당 프로그램의 제일 중요한 원칙
기존 기능과 100% 동일하게 동작이 되어야한다.
해당 프로젝트의 기능은 그대로 유지하고
단순히 pda, 프린트기계, db 오라클 -> mssql로 전환이다.

단 ) 회사코드 및 하드코딩의 경우를 제외한 제외한 나머지는 100% 동일해야한다.

## 프로젝트 목적
1. 기존PDA, 프린터 기계를 Honeywell, BIXOLON  PDA, 프린터 기계로 각각 전환 진행
2. DB 오라클에서 MSSQL로 전환 및 ERP 프로그램 변경으로 인하여 테이블 및 컬럼 전환 (테이블 정보는 D:\HL_ERP\workspace\SGIS_HL_WEBERP 경로의 repository 파일에서 확인 가능)
3. 창고코드와 같은 하드코딩 변경
4. 소스 정리
5. 회사코드 추가
6. 기존기능 그대로 유지

## 프로젝트 개요
- Android PDA 앱 (이마트 출하 관리)
- Java, SQLite(로컬DB), Oracle(서버DB)
- searchType: 0=이마트, 1=생산, 2=홈플러스, 3=도매, 4=비정량, 5=홈플러스비정량, 6=롯데, 7=생산라벨

## 문서 폴더 정의

### 개발 프로세스 흐름
```
오류/ → 개발/ → 테스트/ → 오류/ (재발 시)
```

### 폴더 정의
1. view : view 정리 문서 폴더
	1. 참조 : column/
2. 개발 : 개발 정리 문서 무엇인가를 개발하는 경우 항상 해당 폴더에 문서 작성후 진행한다.
	1. 문서명 : 순번_개발명[오류명(오류 수정시)]
	2. 참조 : 오류/, 소스분석/, 기능/
3. 기능 : 해당 프로그램 기능 정리 문서 폴더
	1. 문서명 : 순번_기능명
	2. 참조 : 소스분석/
4. 소스분석 : 각 파일 별 소스 분석 문서 폴더
	1. 문서명 : 순번_분석명
	2. 참조 : 기능/, 오류/
5. 오류 : 오류가 발생하는 경우 바로 수정을 하는 것이 아닌 해당 폴더에 문서화 진행, 오류 수정시 개발 폴더 하위에 문서 작성
	1. 문서명 : 순번_오류명[테스트시나리오명(테스트시나리오동작시 오류가 발생했을 경우)]
	2. 참조받음 : 개발/
6. 테스트 데이터 : 어떠한 테스트 데이터를 넣었는지 확인 문서
	1. 문서명 : 순번_테스트데이터명
	2. 참조받음 : 테스트/
7. 테스트 : 테스트 시나리오로 작성하여 해당 시나리오대로 동작확인 오류 발생시 오류 폴더 하위에 문서 작성
	1. 문서명 : 순번_테스트시나리오명
	2. 참조 : 테스트데이터/
	3. 오류 발생 시 → 오류/ 문서화
8. column : PDA 컬럼 정의 문서 폴더 (Excel/MD)
	1. 참조 : view/
9. 라벨 : 라벨 출력 관련 문서 폴더
	1. 참조 : 기능/, 소스분석/
10. 참고자료 : 매뉴얼, 기기 관련 참고 자료 폴더
11. 일정 : 일별 개발 일정 문서 폴더
	1. 문서명 : YYYY-MM-DD
	2. 개발 완료 건은 해당 일자 파일의 상태값을 완료(✅)로 처리한다
	3. 일정에 명시되어 있지 않은 완료 건은 "오늘 완료된 작업" 섹션에 추가한다
	4. 참조 : 개발/, 오류/

### 문서 작성 규칙
- 문서 작성 시 "관련 문서" 섹션에 참조하는 다른 폴더의 문서 경로를 명시한다

## 문서 폴더 간 상관관계

### 개발 프로세스 흐름
```
오류/ → 개발/ → 테스트/ → 오류/ (재발 시)
```
- 오류 발견 → 오류/ 문서화 → 개발/ 문서 작성 후 코드 수정 → 테스트/ 시나리오 동작 확인
- 테스트 중 오류 발생 → 오류/ 다시 문서화
- 테스트데이터/는 테스트/ 진행 시 사용한 데이터 기록

### 분석·정보 참조 관계
- 소스분석/ ↔ 기능/ : 코드 레벨 분석과 기능 레벨 동작 정리가 상호 참조
- 개발/ → 소스분석/, 기능/ : 코드 수정 시 양쪽 참조
- view/ ↔ column/ : VIEW 쿼리 구조와 컬럼 매핑 정의가 상호 참조
- 라벨/ → 기능/, 소스분석/ : 라벨 레이아웃·규격이 기능/소스 참조

### 일정 관리
- 일정/ → 개발/, 오류/ : 당일 작업 대상 및 완료 기록

### 폴더별 참조 관계

| 폴더 | 참조하는 폴더 | 참조받는 폴더 |
|------|:------------:|:------------:|
| 오류/ | - | 개발/ |
| 개발/ | 오류/, 소스분석/, 기능/ | 일정/ |
| 테스트/ | 테스트데이터/ | 오류/ (실패 시) |
| 테스트데이터/ | - | 테스트/ |
| 소스분석/ | 기능/, 오류/ | 개발/ |
| 기능/ | 소스분석/ | 개발/ |
| view/ | column/ | 소스분석/ |
| column/ | view/ | 소스분석/ |
| 일정/ | 개발/, 오류/ | - |
| 라벨/ | 기능/, 소스분석/ | - |
| 참고자료/ | - | - |

## 문서 작성 규칙 (Excel/MD)

### Index 기준
- **Index = JSP out.println 출력 순서** (0부터 시작)
- VIEW DDL 컬럼 순서가 아님
- JSP가 직접 JOIN 쿼리를 사용하는 경우에도 SELECT 컬럼 출력 순서 기준

### Index '-' 조건
- JSP에서 전송하지 않는 컬럼 (WHERE/ORDER BY 조건용)
- Java 파싱 클래스에서 temp[]로 파싱하지 않는 컬럼

### 문서 작성 후 필수 검증 (매번 수행)
1. **JSP 확인**: 외부 JSP 프로젝트 경로에서 해당 JSP의 out.println 순서 확인
2. **Index 검증**: JSP out.println 순서와 문서 Index 일치 여부 확인
3. **파싱 검증**: 해당 파싱 클래스의 temp[] 파싱 코드와 Index 일치 여부 확인
	- 출하대상: `ProgressDlgShipSearch.java`
	- 바코드정보: `ProgressDlgBarcodeSearch.java`
	- 계근데이터: `ProgressDlgGoodsWetSearch.java`
4. **미파싱 컬럼**: JSP 전송하나 Java 미파싱이면 Index '-'로 표시
5. **로컬DB 컬럼**: 해당 테이블의 CREATE 쿼리와 실제 컬럼 일치 여부 확인
	- TB_SHIPMENT: `DBHandler.createqueryShipment()`
	- TB_BARCODE_INFO: `DBHandler.createqueryBarcodeInfo()`
	- TB_GOODS_WET: `DBHandler.createqueryGoodsWet()`

### 용도 표기 기준
- JSP 미전달 컬럼: "미사용(JSP미전달·로컬DB없음)"
- Java 미파싱 컬럼: "미사용(Java미파싱)"
- INSERT만 사용: "로컬DB저장용"
- 실제 사용: 구체적 용도 명시

## 주요 파일 경로
- JSP: `D:\PDA\apache-tomcat-8.5.29\apache-tomcat-8.5.29` (외부 Tomcat 프로젝트)
- 컬럼 정의: `app/doc/column/PDA컬럼정의_SGI.xlsx`
- MD 문서: `app/doc/column/`
- ProgressDlgShipSearch: `app/src/main/java/.../common/ProgressDlgShipSearch.java`
- LabelPrintHelper: `app/src/main/java/.../print/LabelPrintHelper.java`
- DBHandler: `app/src/main/java/.../db/DBHandler.java`

## Notion 작업 기록
- 실제 작업(코드 수정·문서 작성·오류 처리·테스트) 완료 시
  Notion `회사 > 📱 PDA > 작업 기록`(캘린더 DB)에 **작업 1건 = 1행**으로 기록한다.
- 행 속성: 작업명(제목), 날짜, 분류(코드수정/문서작성/오류처리/테스트/기타), 상태, 요약.
- 행 페이지 본문에 상세(`## 명령` / `## 결과` / `## 비고`)를 기록한다.
- **문서 작성 작업**인 경우, 해당 작업 기록 행 페이지에 **작성한 문서 파일을 첨부**한다(File Upload API). 첨부에는 인테그레이션 토큰이 필요하므로, 토큰이 없으면 사용자에게 요청하고, 받기 전까지는 행만 등록한다.
- 단순 질문·확인·상태 조회는 제외한다.
- 상세 규칙: `app/doc/참고자료/Notion_작업기록_가이드.md`

## Notion 일정 관리 (전사 공통)
- 위치: Notion `회사 > 일정`(캘린더 DB, 회사 폴더 직속). PDA·WEBERP 등 전 프로젝트 공통 일정.
- 데이터소스: `collection://63d103fb-72d3-40cf-a423-5bb72c4e63a0`
- 사용자가 아침에 "오늘 ~작업을 하겠다"고 하면 → 일정 DB에 **오늘 날짜로 항목 등록**(완료 체크박스 = 미체크).
  - 행 속성: 일정명(제목), 기간(날짜, 생성 시 `date:기간:start` 키), 구분(개발/문서/테스트/회의/기타), 완료(체크박스), 메모.
- 사용자가 해당 작업을 실제로 수행하면 → 그 일정 항목의 **완료 체크박스를 체크**(`완료` = `__YES__`)하고, 작업 기록 DB에도 기록한다.

## Notion 개발 파이프라인 (하네스)
- 상태머신: Notion `회사 > PDA > 개발` DB(데이터소스 `collection://acaceae4-52ef-491d-bb81-f7564535d6fb`)의 `단계`(요청→개발문서→개발중→테스트→완료).
- 흐름: ① 사용자가 개발 DB 행 본문에 요구 작성(단계=요청) → ② 개발문서 초안+검증 → ③ 개발 → ④ 코드검증 → ⑤ 테스트문서+테스트 → ⑥ git commit(단계=완료).
- **가드레일**: "원본 100% 동일" 위반은 검증 게이트에서 차단·반려. 코드 수정·commit은 **사람 승인 게이트**(자동 워크플로우에 넣지 않음).
- Workflow 스크립트(`.claude/workflows/`):
  - `notion-dev-pipeline` — 요구분석→개발문서초안(dev-guide)→문서검증(doc-consistency+적대적). 호출: `Workflow({name:'notion-dev-pipeline', args:{rowId:'<개발DB행id>'}})`
  - `notion-code-verify` — 변경 diff에 code-verifier+original-comparator 적대적 검증(읽기전용). 호출: `Workflow({name:'notion-code-verify', args:{base:'<선택: git ref>'}})`
- 문서 산출물(개발문서/테스트문서)은 로컬 app/doc/ 원본 → hook이 Notion 작업기록에 자동 미러. 개발 DB 행의 `개발문서`/`테스트문서` URL로 연결한다.

## 커밋 메세지 작성
1. 문서 기반으로 step 별 개발 진행사항 커밋의 경우
    문서명 : step명
    ex) 12_LabelPrintHelper_분리_계획 ### Step 9: 통합 테스트

2. 문서 커밋의 경우
    문서 : 문서 주제
    문서: 개발 가이드 파일명 번호 부여 및 내용 수정

3. JSP 파일 커밋의 경우
    JSP 파일명 [수정/개발]
    search_production.jsp [수정]

-- 커밋 후 PUSH 까지 진행

## 외부 프로젝트 경로
JSP 프로젝트 경로
목적 : JSP 파일에 관해 데이터를 읽을때 해당 경로에서 확인하라
D:\PDA\apache-tomcat-8.5.29\apache-tomcat-8.5.29

JSP 원본 프로젝트 경로
목적 : JSP 원본 프로젝트의 경로이다.
D:\PDA\apache-tomcat-8.5.29\apache-tomcat-7.0.78_PDA_IN(원본)

APP 원본 프로젝트 경로
목적 : 해당 프로그램은 원본 프로그램과 기능이 100% 동일해야함으로 비교하기 위한 용도이다.
D:\PDA\PDA-INNO(원본)

HL_ERP 프로젝트 경로
목적 : ERP 파일에 관해 데이터를 읽을때 해당 경로에서 확인하라
D:\HL_ERP\workspace\SGIS_HL_WEBERP

단 읽을때는 나에게 허락을 받지 않아도 괜찮다.


## 서브 에이전트

### 에이전트 목록 (8개)

| # | 에이전트명 | name (호출명) | 역할 |
|:-:|----------|:----------:|------|
| ① | 일정+커밋 | `schedule-commit` | 커밋+푸시+체크리스트+일정 반영+일정 생성 (허브) |
| ② | 오류 문서 | `error-doc` | 오류/ 템플릿 기반 문서 생성 (순번 자동) |
| ③ | 개발 가이드 | `dev-guide` | 개발/ 영향 분석+step 설계 문서 생성 |
| ④ | 기능/소스분석 문서 | `analysis-doc` | 기능/, 소스분석/ 문서 생성 |
| ⑤ | 코드 검증 | `code-verifier` | 인덱스+파라미터+WHERE절+컴파일+로컬DB 정합성 검증+동작 시뮬레이션 |
| ⑥ | 원본 비교 | `original-comparator` | PDA-INNO(원본)과 기능 동일성 비교 |
| ⑦ | 테스트 | `test-manager` | 테스트 시나리오 작성+테스트 데이터 문서화 |
| ⑧ | 문서 정합성 | `doc-consistency` | 문서 간 번호/이름/경로/WF 호출 순서 일치 검증 |

### 에이전트 파일 경로
`.claude/agents/*.md`

### 워크플로우 지시 방법
- "WF-N으로 진행하라"로 해당 워크플로우 전체를 순서대로 진행
- "오류 문서 작성하고 검증하고 커밋까지 해라" 같은 연쇄 지시도 가능
- 코드 수정은 서브 에이전트가 아닌 메인 에이전트 + 사용자 지시로 진행

### 주요 워크플로우

| WF | 시나리오 | 흐름 |
|:--:|---------|------|
| 1 | 오류 발견 → 수정 → 커밋 | ② → ⑧ → ① → ③ → ⑧ → ① → [메인 코드 수정] → ⑤⑥ → ① → ⑦ |
| 2 | 기능/소스분석 문서 → 커밋 | ④ → ⑧ → ① |
| 3 | 단순 코드 수정 → 커밋 | ③ → ⑧ → ① → [메인 코드 수정] → ⑤⑥ → ① |
| 5 | 하루 시작/종료 | ① |

### 관련 문서
- `app/doc/기능/13_서브에이전트.md` — 에이전트 기능 정의, 상관관계, 워크플로우
- `app/doc/개발/34_서브에이전트_적용.md` — 에이전트 구현 개발 가이드
- `app/doc/참고자료/서브에이전트_활용_가이드.md` — 호출 방법, WF별 예시, 결과 확인
- `app/doc/참고자료/오류패턴_분석.md` — 오류 20건 패턴 분류, searchType별 사전 체크리스트