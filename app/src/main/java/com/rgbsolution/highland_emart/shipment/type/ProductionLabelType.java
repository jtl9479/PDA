...
public class ProductionLabelType implements ShipmentType {
     ...
     *
     * <p>이 타입은 {@code setPrinting_prod} 로 생산 라벨을 찍는다(원본 1666).</p>
     */
    @Override
    public void onWeightConfirmed(String weight_str, double weight_double, String making_date, String box_serial) {
        ...
        if (Common.print_bool) {
            // 원본 1666 : 라벨 분기 — 생산라벨(7) 고정이라 searchType 조건만 제거
            Log.d(TAG, "===========생산 출력 시작 ================");
            a.labelPrintHelper.setPrinting_prod(weight_double, a.arSM.get(a.current_work_position), false, a.printerCallback);
        }
    ...
    public void reprintLabel(String print_weight_str, String making_date, String box_order, int select_position) {
        // 원본 977 : searchType 7 생산라벨 경로. 이 타입 고정이라 조건문만 제거
                    a.labelPrintHelper.setPrinting_prod(Double.parseDouble(print_weight_str), a.arSM.get(select_position), true, a.printerCallback);
    }
    ...
    /**
     * 서버 전송 — 원본 ProgressDlgShipmentSend.doInBackground 의 <b>일괄 누적</b>(현재 2412~2515) 이관 (개발67 Step 6)
     *
     * <p>목록 조회(현재 2312~2325)는 Activity 에 남아 있고, 이 메서드는 조회 결과를 받아 전송만 한다.
     * 일괄은 전 건을 {@code ##} 로 이어붙여 {@code insert_goods_wet_production.jsp} 로 한 번에 보내고,
    ...
    public String send(Context mContext, ArrayList<Goodswets_Info> list_send_info, ArrayList<Shipments_Info> arSM) {
        ...
        try {
            ...
            if(sendOrNot){
                //전문전송..
            // 원본 2458~2460 : 생산(1)·생산라벨(7) 전송 — 이 타입 고정이라 조건문만 제거
            Log.i(TAG, "===================send packet 확인==================" + packet);
