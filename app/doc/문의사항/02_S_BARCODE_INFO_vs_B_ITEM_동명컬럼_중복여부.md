...
> 저희 자체 ERP 에서는 품목의 등록을 여러군데서, 품목의 특성별로 다른 테이블에서 관리해서 그렇습니다. SGI 에서는 품목을 하나의 테이블에 입력하는 것으로 아는데, 하나로 할 수 있으면 하시면 됩니다.

### 결론

- **C0705 화면 1곳에서 바코드 파싱 규칙 관리해도 됨**
- MSSQL 전환 후 SGI ERP의 단일 품목 테이블(`CO_품목코드`)에서 관리하는 방향으로 진행
