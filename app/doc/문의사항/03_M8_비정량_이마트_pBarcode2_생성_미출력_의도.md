...
이마트 비정량(M8) 라벨 출력 시 **pBarcode2(물류코드 기반 바코드)가 코드상 생성은 되지만 실제 라벨에는 인쇄되지 않습니다**. 지워도 상관이 없습니까?
...
**이마트 비정량(M8) 라벨에서 `pBarcode2`(물류코드 기반 바코드)를 코드에서 완전히 제거해도 됩니까?**

- 현재 코드상 `pBarcode2`는 **생성만 되고 라벨에 인쇄되지 않습니다**.
- 즉 **pBarcode2 생성 코드를 삭제해도 실운영에 영향이 없습니까**?
- 아니면 **향후 pBarcode2 인쇄가 필요하므로 남겨두어야 합니까**?
