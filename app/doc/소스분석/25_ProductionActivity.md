...
### 개요
ProductionActivity.java에서 불필요한 주석 처리된 코드와 미사용 변수를 삭제하였습니다.

### 1. 삭제된 주석 처리 코드

| 원래 위치 | 삭제된 코드 | 설명 |
|-----------|-------------|------|
| 86-87행 | `/*edit_pp_code.setText("22872"); edit_packer_code.setText("30360");*/` | 테스트용 코드 |
| 191행 | `//scan_flag = true;` | 주석 처리된 코드 |

### 2. 삭제된 미사용 변수

| 변수명 | 타입 | 원래 위치 | 클래스 | 삭제 사유 |
|--------|------|-----------|--------|-----------|
| `receiveData` | `String` | 411행 | SelectWeightFromTo | 선언만 되고 사용되지 않음 |
...
- 예: `// 스캔한 바코드 정보`, `//  1 바코드스캔 , 2 수기입력`, `//혹시 모르니 프로그램 들어갈 때 한번 더 지워줌` 등
