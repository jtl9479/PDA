...
### 개요
LoginActivity.java를 분석하여 불필요한 주석 처리된 코드와 미사용 변수를 확인하였습니다.
**※ 정리 작업은 수행하지 않음 (기록만 함)**

### 1. 주석 처리된 코드 (정리 대상)

| 위치 | 내용 | 설명 |
|------|------|------|
| 127행 | `//exitDialog();` | 주석 처리된 메서드 호출 |
| 231-232행 | `// 테스트 시 제외` + `//new ProgressDlgShipSearch(...)` | 주석 처리된 코드 + 설명 |
| 401행 | `//mBixolonPrinter.printTestLabel();` | 중복 호출 주석 (402행에서 동일 호출) |

### 2. 미사용 변수/필드 (정리 대상)

| 항목 | 타입 | 위치 | 삭제 사유 |
|------|------|------|-----------|
| `mPrintService` | `BluetoothPrintService` | 52행 | 선언만 되고 전혀 사용되지 않음 |
| `mHandler` | `Handler` | 340-352행 | 선언만 되고 사용되지 않음 (빈 Handler) |

### 3. 중복 import (정리 대상)

| 위치 | 내용 | 설명 |
|------|------|------|
| 37행 | `import java.security.MessageDigest;` | 29행에서 이미 import됨 |
