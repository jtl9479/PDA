...
- 범위: Step 1~8에서 변환된 모든 코드
- 용도: Woosim → SLCS 변환 완료 후 전체 빌드 및 실제 기기 테스트
- 주의할 점:
  - APK 빌드 후 PDA 기기에 설치 필요
  - Bixolon 프린터 블루투스 연결 필요
  - 각 라벨 유형별 실제 인쇄 테스트 필요
- 테스트 항목: 6개

| # | 테스트 항목 | 설명 | 담당 |
|---|------------|------|------|
| 1 | 전체 빌드 | APK 생성 확인 | 개발자 |
| 2 | PDA 기기 설치 | APK 설치 및 실행 | 사용자 |
| 3 | 프린터 연결 | Bixolon 블루투스 연결 | 사용자 |
| 4 | 이마트 라벨 인쇄 | searchType 0, E0~E3 | 사용자 |
| 5 | 홈플러스 라벨 인쇄 | searchType 2, 5 | 사용자 |
| 6 | 롯데 라벨 인쇄 | searchType 6, L0 | 사용자 |

**Part 2. 테스트 계획**
- 빌드 방식: gradlew assembleDebug → APK 생성
- 설치 방식: adb install 또는 직접 설치
- 테스트 순서:
  1. APK 빌드 및 생성 확인
  2. PDA 기기에 APK 설치
  3. 앱 실행 및 로그인
  4. Bixolon 프린터 블루투스 연결
  5. 각 라벨 유형별 인쇄 테스트
- 주의사항:
  - 빌드는 개발자가 수행
  - 기기 설치 및 인쇄 테스트는 사용자가 수행 (PDA/프린터 필요)

**체크리스트**
- [ ] Part 1: 분석 완료 확인
- [ ] Part 2: 테스트 계획 확인
- [ ] Part 3: 전체 빌드 (APK 생성)
- [ ] Part 4: PDA 기기 설치 (사용자)
- [ ] Part 5: 프린터 연결 테스트 (사용자)
- [ ] Part 6: 라벨 인쇄 테스트 (사용자)
- [ ] Part 7: 테스트 결과 기록
- [ ] Part 8: 변경 내용 작성
