...
### 사전 준비
- [ ] 원본 코드 백업
- [ ] 모든 searchType 기능 동작 확인

### 1단계: searchType 상수 정의
- [ ] 8개 상수 선언 추가
- [ ] 기존 `"0"` ~ `"7"` 문자열을 상수로 교체
- [ ] 테스트: 기존과 동일하게 동작 확인

### 2단계: 날짜 포맷팅 메서드 추출
- [ ] `formatDateYYYYMMDD()` 메서드 추가
- [ ] DatePickerDialog 리스너에서 호출
- [ ] 8개 다운로드 버튼에서 호출
- [ ] 테스트: 날짜 출력 형식 확인

### 3단계: 다운로드 로직 메서드 추출
- [ ] `downloadShipmentList()` 메서드 추가
- [ ] 8개 다운로드 버튼 수정
- [ ] 테스트: 모든 다운로드 기능 동작 확인

### 4단계: 계근입력 로직 메서드 추출
- [ ] `startWeighing()` 메서드 추가
- [ ] 8개 계근입력 버튼 수정
- [ ] btnProdWet4print는 Log.i 유지 확인
- [ ] 테스트: 모든 계근입력 기능 동작 확인

### 5단계: 기타 메서드 추출
- [ ] `showDatePicker()` 메서드 추가
- [ ] `deleteShipmentData()` 메서드 추가
- [ ] `startProductionActivity()` 메서드 추가
- [ ] `startSettingActivity()` 메서드 추가
- [ ] onClick 메서드에서 호출하도록 수정
- [ ] 테스트: 각 버튼 동작 확인
