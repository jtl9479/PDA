...
**미해결 — 생산라벨(7) 위임 대상 없음** (Step 7에서 새로 드러남)

| 항목 | 내용 |
|---|---|
| 현상 | `onCreate`(431~434)는 생산(1)·생산라벨(7) 모두 Factory 대상에서 제외해 `shipmentType` 이 `null` 인데, `setBarcodeMsg` 는 생산(1)만 걸러낸다. searchType 7 로 스캔하면 `shipmentType.onBarcodeScanned(msg)` 에서 NPE |
| 원본 | 원본 `ShipmentActivity.setBarcodeMsg` 는 분기 없이 본문을 실행하므로 7도 정상 동작. `setBarcodeMsg` 본문 안에는 7 전용 분기가 없고, searchType 게이트가 전부 거짓이라 **도매·롯데와 동일한 경로**를 탄다 |
| Step 6까지 | 7은 과도기 분기를 통과해 원본 본문을 탔으므로 문제가 없었다. 본문을 삭제한 Step 7에서 처음 생긴 경로다 |
| 현재 조치 | §4.4 원안대로 무조건 위임을 유지했다. 7은 `MainActivity` 에서 미사용(2026-08-04 제외 결정, `search_production_4label.jsp` 삭제)이라 대상 리스트를 받을 수 없어 진입이 막혀 있다 |
| 주의 | `setBarcodeMsgProduction` 으로 보내는 것은 동작이 다르다. 그 메서드는 ITEM_TYPE W/HW·B 블록과 킬코이·센터명 검증을 이미 제거했고, 7 은 원본에서 그 블록들을 탄다 |
