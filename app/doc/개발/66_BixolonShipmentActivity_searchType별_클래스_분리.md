...
com/rgbsolution/highland_emart/shipment/type/
  ...
  common/                         ← Step 12 결과 **생성하지 않음**. 기준 3개를 통과하는 블록이 없었다
```
...
| 타입 파일 간 중복 | – | 존재(허용) | Step 12 판정 결과 **유지**(기준 미충족) |
...
- [x] Part 3: 변환 수행
- [x] Part 4: 컴파일 확인 — `gradlew assembleDebug` → `BUILD SUCCESSFUL` (2026-09-16, 4s)
- [ ] Part 5: 단위테스트 — 실기기 확인 필요 (롯데 스캔 → 계근 → 라벨)
- [ ] Part 6: 회귀테스트 — 실기기 확인 필요 (이마트·홈플러스·도매 무영향)

**Part 6. 변경 내용** (완료):
- **무엇을**: 원본 `setBarcodeMsg` 본문을 `LotteType.onBarcodeScanned` 로 기계적 복사. Activity 과도기 분기에 롯데 추가
- **왜**: 접는 조건이 도매와 동일해 Step 2에서 확정한 절차의 재현성을 확인하는 단계다
- **어떻게**: Step 2와 같은 조건 5곳을 접었다. 결과가 `WholesaleType` 과 같더라도 **판단으로 묶지 않고 복사**했다. 공통화는 Step 12에서 diff로 수행한다

**접은 조건** — Step 2와 동일 (원본 1213 · 1337 중복확인 제외, 1302 트레이더스 소비기한, 1451 · 1513 LB 자릿수). 총 23줄
...
|---|---|
| 빌드 | `BUILD SUCCESSFUL` (4s) |
| 원본 대조 | 정규화 후 원본 333줄 → 신규 316줄. **삭제 26 / 추가 9**, 산술 일치 |
| 추가 9줄 내역 | 래퍼 호출 2 + 재귀 1 + 주석 6. **로직으로 추가된 줄 0** |
| `WholesaleType` 대조 | "도매/롯데" 표기만 정규화하면 **본문 완전 일치** — 복사 누락·오타 없음 |
| Activity 변경 | 과도기 분기 조건 1줄 추가 + 주석 문구 1줄(추가 3 / 삭제 2). **로직 변경 없음** |
| 박스순번 | `lotte_TryCount` 는 이 Step 범위 밖. Step 8에서 이관 |
...
- [x] Part 3: 변환 수행
- [x] Part 4: 컴파일 확인 — `gradlew assembleDebug` → `BUILD SUCCESSFUL` (2026-09-16, 4s)
- [ ] Part 5: 단위테스트 — **테스트 데이터 확보 후** 실기기 확인
- [ ] Part 6: 회귀테스트 — **테스트 데이터 확보 후** 실기기 확인

**Part 6. 변경 내용** (완료):
- **무엇을**: 원본 `setBarcodeMsg` 본문을 `HomeplusType.onBarcodeScanned` 로 기계적 복사. Activity 과도기 분기에 홈플러스 추가
- **왜**: 정량(2)은 접는 조건이 도매·롯데와 동일해 같은 절차로 이관할 수 있다. 비정량(5)과의 차이(중복확인·INSERT·전송)는 이 Step 범위가 아니다
- **어떻게**: Step 2·3과 같은 조건 5곳(23줄)을 접었다. `ITEM_TYPE B` 블록은 상수 주석만 "홈플러스 비정량"일 뿐 searchType 게이트가 없으므로 **그대로 유지**했다(§1.4)
...
| 원본 대조 | 정규화 후 원본 333줄 → 신규 316줄. **삭제 26 / 추가 9**, 산술 일치 |
| 추가 9줄 내역 | 래퍼 호출 2 + 재귀 1 + 주석 6. **로직으로 추가된 줄 0** |
| `WholesaleType` 대조 | 타입 표기만 정규화하면 **본문 완전 일치** |
| Activity 변경 | 과도기 분기 조건 1줄 추가 + 주석 문구 1줄(추가 3 / 삭제 2). **로직 변경 없음** |
| 미이관 | INSERT(MaxBoxOrder)·라벨·건별 전송은 Step 8·10 범위 |

> **실기기 검증 보류** — 운영 DB에 홈플러스 데이터가 0건일 수 있다. 코드 이관은 완료했으나 Part 5·6은 테스트 데이터 확보 후 수행한다.
...
- [x] Part 3: 변환 수행
- [x] Part 4: 컴파일 확인 — `gradlew assembleDebug` → `BUILD SUCCESSFUL` (2026-09-16, 4s)
- [ ] Part 5: 단위테스트 — 실기기 확인 필요 (동일 바코드 연속 스캔 허용)
- [ ] Part 6: 회귀테스트 — 실기기 확인 필요 (정량 계열 중복 거부 유지 확인)

**Part 6. 변경 내용** (완료):
- **무엇을**: 원본 `setBarcodeMsg` 본문을 `HomeplusNonfixedType.onBarcodeScanned` 로 기계적 복사. Activity 과도기 분기에 홈플러스 비정량 추가
- **왜**: **접는 방식이 처음으로 달라지는 타입**이다. 1213·1337 조건이 참이므로 앞선 3개 타입과 반대로 처리해야 한다
- **어떻게**: 1213·1337은 **조건문만 제거하고 `dup = false;` 는 남겼다.** 중복검사 조회(`duplicatequeryGoodsWet_check` 2회, `duplicatequeryGoodsWet` 1회)와 뒤따르는 `if (dup)` 블록은 그대로 두었다

**접은 조건 — 앞선 타입과 다른 부분**

| 원본 라인 | 조건 | 도매·롯데·홈플러스(2) | 홈플러스 비정량(5) |
|---|---|---|---|
| 1213 · 1337 | `NONFIXED \|\| HOMEPLUS_NONFIXED` | 거짓 → 블록 전체 삭제(3줄씩) | **참 → 조건문만 삭제(2줄씩), `dup = false;` 유지** |
| 1302 | `EMART` | 내부 블록 삭제 (9줄) | 동일 |
| 1451 · 1513 | `EMART` | else 경로만 유지 (4줄씩) | 동일 |
...
|---|---|---|
| 빌드 | — | `BUILD SUCCESSFUL` (4s) |
| 원본 대조 | 삭제 24 / 추가 9 | **삭제 24 / 추가 9** (333 → 318, 산술 일치) |
| 삭제 목록에 `dup = false;` | 없어야 함 | **없음** |
| `dup = false;` 잔존 | 2 | **2** |
| 중복검사 조회 호출 잔존 | 3 | **3** |
| `WholesaleType` 대조 | `dup = false;` 2줄 + 주석 차이 | **정확히 그 2줄 + 주석뿐** |
| 로직 추가 | 0 | **0** (추가는 래퍼 2 + 재귀 1 + 주석 6) |
| Activity 변경 | 과도기 분기 1줄 | 추가 3 / 삭제 2, 로직 변경 없음 |

> **조회 생략 금지가 코드로 지켜졌다.** 원본은 조회한 뒤 결과를 `false`로 덮는다. 조회를 건너뛰면 `DBHandler` 내부 로그가 사라져 동작이 달라진다(§1.5).
...
- [x] Part 4: 컴파일 확인 — `gradlew assembleDebug` → `BUILD SUCCESSFUL` (2026-09-16, 4s)
- [ ] Part 5: 단위테스트 — 실기기 확인 필요 (동일 바코드 연속 스캔 허용)
- [ ] Part 6: 회귀테스트 — 실기기 확인 필요 (이마트 정량(0) 무영향 확인)

**Part 6. 변경 내용** (완료):
- **무엇을**: 원본 `setBarcodeMsg` 본문을 `EmartNonfixedType.onBarcodeScanned` 로 기계적 복사. Activity 과도기 분기에 이마트 비정량 추가
- **왜**: 이름은 이마트지만 `SEARCH_TYPE_EMART` 는 `"0"` 이므로 `"4"` 는 이마트 전용 분기에 해당하지 않는다. 접는 방식은 홈플러스 비정량(5)과 같다
- **어떻게**: 1213 · 1337은 조건문만 제거하고 `dup = false;` 유지. 1302 · 1451 · 1513은 **거짓이므로 접었다**

**접는 조건 — 이름에 속지 말 것**

| 원본 라인 | 조건 | 이마트 정량(0) | 이마트 비정량(4) |
|---|---|:-:|:-:|
| 1213 · 1337 | `NONFIXED \|\| HP_NONFIXED` | 거짓 | **참** → 조건문만 제거 |
| 1302 | `EMART`("0") 트레이더스 검증 | 참 | **거짓** → 내부 블록 삭제 |
| 1451 · 1513 | `EMART`("0") LB 자릿수 | 참 | **거짓** → else 경로 |
...
| 빌드 | — | `BUILD SUCCESSFUL` (4s) |
| 원본 대조 | 삭제 24 / 추가 9 | **삭제 24 / 추가 9** (333 → 318, 산술 일치) |
| 삭제 목록의 `dup = false;` | 없어야 함 | **없음** |
| 삭제 목록의 `SEARCH_TYPE_EMART` 조건 | 3건(1302·1451·1513) | **3건** |
| `dup = false;` 잔존 | 2 | **2** |
| 중복검사 조회 호출 | 3 | **3** |
| `HomeplusNonfixedType` 대조 | 주석만 차이 | **주석 3줄뿐, 로직 완전 일치** |
| 로직 추가 | 0 | **0** |
| Activity 변경 | 과도기 분기 1줄 | 추가 3 / 삭제 2, 로직 변경 없음 |

> 전부 매칭(원본 2150)은 이 타입 전용이지만 **상품 매칭 축**이라 Step 9에서 이관한다. 이 Step에서는 건드리지 않았다.
...
- [ ] Part 5: 단위테스트 — 실기기 확인 필요 (이마트 정량 스캔 → 계근 → 라벨, 트레이더스 센터 소비기한 검증)
- [ ] Part 6: 회귀테스트 — 실기기 확인 필요 (도매·롯데·홈플러스·비정량 2종 무영향, 생산(1) 무영향)

**Part 6. 변경 내용** (완료):
- **무엇을**: 원본 `setBarcodeMsg` 본문을 `EmartType.onBarcodeScanned` 로 이관하고, Activity 본문 411줄을 삭제해 위임 1줄로 교체. Step 2~6 과도기 분기 제거
- **왜**: 마지막 타입 이관. 6종 전부 위임으로 정리되어 Activity에는 생산(1) 분기와 위임만 남는다
- **어떻게**: 1213 · 1337은 **거짓이므로 블록 삭제**(`dup = false;` 도 함께 삭제). 1302는 **참이므로 조건문만 제거하고 내부 유지**. 1451 · 1513은 **참이므로 if 본문(`item_pow` 경로)만 남기고 else 삭제**

**접는 조건 — 이마트 정량(0)은 다른 5종과 반대**

| 원본 라인 | 조건 | 이마트 정량(0) | 도매(3) · 롯데(6) · 홈플러스(2) |
|---|---|:-:|:-:|
| 1213 · 1337 | `NONFIXED \|\| HP_NONFIXED` | 거짓 → 블록 삭제 | 거짓 → 블록 삭제 |
| 1302 | `EMART`("0") 트레이더스 검증 | **참** → 조건문만 제거, 내부 유지 | 거짓 → 내부 블록 삭제 |
| 1451 · 1513 | `EMART`("0") LB 자릿수 | **참** → if 본문(`item_pow`) 유지 | 거짓 → else 경로(`/100`) |
...
|---|---|---|
| 빌드 | — | `BUILD SUCCESSFUL` |
| 원본 본문 대조 (정규화 후) | 접은 조건 5곳만 차이 | **5곳만 차이** (325 → 309줄, 그 외 로그·순서·조건 전부 일치) |
| `WholesaleType` 로직 대조 | 3곳 차이 (1302 복원 · 1451 · 1513) | **3곳** (트레이더스 7줄 추가, LB 자릿수 2줄 교체) |
| Activity 본문 삭제 | 411줄 | **411줄** (3,492 → 3,081줄) |
| `EmartType` | 74 → 약 476줄 | **476줄** |
| 과도기 분기 | 제거 | **제거** (`isMigrated()` 전체 소스 0건) |
| 다른 타입 파일 5종 | 변경 0 | **diff 0줄** |
| 로직 추가 | 0 | **0** |

**생산라벨(7) 경로 — 검토 후 §4.4 원안 유지로 종결** (2026-09-17)

검증 과정에서 "searchType 7 로 스캔하면 `shipmentType` 이 `null` 이라 NPE" 라는 지적이 나왔으나,
**7 은 진입 경로가 없어 해당하지 않는다.** 아래 3가지가 모두 막혀 있다.

| # | 차단 지점 | 근거 |
|:-:|---|---|
| 1 | 화면 | `activity_main.xml` 406 — 생산라벨 버튼 2개(`btnproductionlist4print` · `btnProdWet4print`)를 감싼 `TableRow` 가 `android:visibility="gone"` |
| 2 | 진입점 | `Common.searchType` 에 `"7"` 을 넣는 곳은 위 두 버튼의 핸들러(`MainActivity` 301 · 347)뿐이다 |
| 3 | 상태 | `Common.searchType` 은 `public static String searchType = "0"` 으로, 저장·복원 없이 실행마다 `"0"` 에서 시작한다. 과거 세션 값이 남지 않는다 |

추가로 서버측 `search_production_4label.jsp` 도 삭제되어(2026-08-04 제외 결정) 대상 리스트를 받을 수 없다.

**단서** — 위 1번(`visibility="gone"`)이 근거다. **그 `TableRow` 를 다시 노출시키면 NPE 가 살아난다.**
생산라벨을 되살릴 경우 `setBarcodeMsg` 의 위임 앞에 7 처리를 먼저 추가해야 한다.

| 항목 | 내용 |
|---|---|
| 원본 동작 | 원본 `ShipmentActivity.setBarcodeMsg` 는 분기 없이 본문을 실행한다. 본문에 7 전용 분기가 없고 searchType 게이트가 전부 거짓이라 **도매·롯데와 동일한 경로**를 탄다 |
| 되살릴 때 | 7 전용 위임 대상이 필요하다. `setBarcodeMsgProduction` 으로 보내면 **동작이 다르다** — 그 메서드는 ITEM_TYPE W/HW·B 블록과 킬코이·센터명 검증을 이미 제거했고, 7 은 원본에서 그 블록들을 탄다 |
| Step 8 · 9 | 위임 게이트를 `shipmentType != null` 로 두고 Activity 본문을 남겨서, 7 이 살아나도 기존 본문을 탄다. 문제가 되는 곳은 `setBarcodeMsg` 한 곳뿐이다 |
...
> **라인 번호 정정** — Step 0 직전 파일과 내용 대조로 확인한 실제 위치는 전부 매칭 **2149~2161**, 구간 추출 **2121**(vs `find_work_info_barcodeGoods` **2184**), 항상 참 조건 **2065 · 2081**, 일반 매칭 **2130** 이다. 위 표의 값은 각각 +1 어긋나 있다. 코드 주석은 확인된 값을 쓴다.
...
- [x] Part 3: 변환 수행
- [x] Part 4: 컴파일 확인 — `gradlew assembleDebug` → `BUILD SUCCESSFUL` (2026-09-17)
- [ ] Part 5: 단위테스트 — 실기기 확인 필요 (타입별 상품 스캔 · 상품코드 스캔, 이마트 비정량 `pp_code` 누적 문자열 logcat 대조)
- [ ] Part 6: 회귀테스트 — 실기기 확인 필요 (생산(1) 상품 매칭, 출하대상 조회 후 상품명 표시)

**Part 6. 변경 내용** (완료):
- **무엇을**: `find_PackerProduct` · `find_PackerProductBarcodeGoods` · `find_work_info` · `find_work_info_barcodeGoods` 를 타입 파일 6개로 이관. `findPackerProduct(barcode, workFlag)` 하나로 진입하고 `find_work_info` 계열은 타입 파일의 private 메서드가 된다
- **왜**: 차이 1곳(전부 매칭)이 `find_work_info` 안에 있어 4번 타입만 다르게 동작한다
- **어떻게**: 전부 매칭 블록은 `EmartNonfixedType` 에서만 **조건문만 제거**하고 위치를 그대로 두었다(일반 매칭 뒤). 나머지 5종은 블록을 삭제했다. 구간 추출 차이(길이 체크 유무)는 **두 메서드를 합치지 않고** 그대로 유지했다

**Activity 원본 메서드는 남겼다**

| 호출처 | 이유 |
|---|---|
| `setBarcodeMsgProduction`(생산 1) | 생산 경로는 타입 구현체를 쓰지 않는다 |
| `ProgressDlgShipSelect.onPostExecute` | 타입 구분 없이 `find_work_info` 를 호출한다. searchType 4 분기도 그대로 남아 있어야 한다 |

> 그래서 Activity 사본과 타입 파일 사본이 공존한다. 공통화는 Step 12에서 diff로 판단한다.
...
|---|---|---|
| 빌드 | — | `BUILD SUCCESSFUL` |
| `find_work_info` 대조 (5종) | 전부 매칭 13줄 삭제 / 추가 0 | **삭제 13 / 추가 0** |
| `find_work_info` 대조 (4번) | `if` 래퍼 2줄만 삭제 | **삭제 2 / 추가 0** (본문·순서 그대로) |
| `find_work_info_barcodeGoods` 대조 (6종) | 차이 없음 | **diff 0줄** |
| 두 메서드 병합 | 없어야 함 | **없음** (길이 체크 유무 유지) |
| 호출처 치환 | 6종 | **6종** (`a.find_PackerProduct*` 실호출 잔존 0) |
| 스텁 잔존 | 1개(Step 10 전송) | **각 파일 1개** |
| Activity 변경 | 필드 2개 공개만 | **`edit_product_name` · `edit_product_code` 공개만**, 로직 변경 없음 |
| 생산(1)·생산라벨(7) | 무영향 | **무영향** (Activity 사본 그대로 호출) |

**주석 라인 번호 정정** — Step 8 · 9 에서 코드 주석에 적었던 `원본 N` 값이 현재 파일 기준이었다. §7 규칙대로 **Step 0 이전 원본 기준**으로 고쳤다(Step 0 직전 파일과 내용 대조로 확인).

| 구분 | 수정 전(현재 파일 기준) | 수정 후(원본 기준) |
|---|---|---|
| INSERT 분기 | 1537 · 1541 · 1551 | **1918 · 1922 · 1932** |
| 중량 반올림 4곳 | 1559 · 1573 · 1591 · 1601 | **1940 · 1954 · 1972 · 1982** |
| 계근 라벨 | 1624~1641 | **2005~2022** |
| 재출력 라벨 | 947 · 949 · 957 | **939 · 941 · 949** |
| 롯데 박스순번 초기화 | 2112 | **2549** |
| 상품 매칭 | 1698 · 1715 · 1743 · 1752 · 1771 · 1806 | **2065 · 2081 · 2121 · 2130 · 2149 · 2184** |
...
- [x] Part 3: 변환 수행
- [x] Part 4: 컴파일 확인 — `gradlew assembleDebug` → `BUILD SUCCESSFUL` (2026-09-17)
- [ ] Part 5: 단위테스트 — **소량 데이터로 먼저 확인 필요** (타입별 전송 → 서버 적재 → 로컬DB `SAVE_TYPE` 갱신)
- [ ] Part 6: 회귀테스트 — 실기기 확인 필요 (생산(1) 전송 `insert_goods_wet_production.jsp` 무영향)

**Part 6. 변경 내용** (완료):
- **무엇을**: `ProgressDlgShipmentSend.doInBackground` 의 전송 본문을 타입 파일 6개의 `send(...)` 로 이관. Activity는 목록 조회까지 하고 위임한다
- **왜**: 건별(0 · 2 · 6)과 일괄(3 · 4 · 5)은 패킷 구성·전송 횟수·후처리가 전혀 다른데 한 메서드에 `if / else if` 로 묶여 있었다
- **어떻게**: 조회 구간(원본 2919~2937)은 Activity에 남기고, 그 뒤에 `shipmentType != null` 이면 `send(mContext, list_send_info, arSM)` 로 위임한다. 기존 본문은 생산(1)·생산라벨(7)용으로 **삭제하지 않았다**

**타입별 전송 방식**

| 타입 | 방식 | 패킷 | 전송 URL |
|---|---|---|---|
| 이마트(0) · 홈플러스(2) · 롯데(6) | **건별 루프** (원본 2938~3014) | `…::BOX_ORDER::GI_L_ID` (1건 = 1패킷) | `insert_goods_wet.jsp` |
| 도매(3) · 이마트비정량(4) · 홈플비정량(5) | **일괄 누적** (원본 3015~3117) | `…::BOX_ORDER::GI_L_ID##` 이어붙임 | `insert_goods_wet_new.jsp` |
| 생산(1) · 생산라벨(7) | Activity 기존 본문 | 일괄과 동일 | `insert_goods_wet_production.jsp` |

> 건별의 디비접속 설정 분기(원본 2966~2974)는 **네 갈래가 전부 같은 호출**이었다. 세 타입 모두 같은 한 줄로 접힌다.

**publishProgress 미이관** — 루프 안의 `publishProgress("progress", …)`(원본 2986 · 3090)는 옮기지 않았다.

| 항목 | 내용 |
|---|---|
| 이유 | `send(...)` 는 `AsyncTask` 바깥이라 `publishProgress` 를 호출할 수 없다 |
| 근거 | `onProgressUpdate` 는 `super.onProgressUpdate(values)` 호출뿐인 **no-op** 이다. 원본부터 그랬고 진행률 표시는 원래도 갱신되지 않았다 |
| 판정 | 코드검증·원본비교 양쪽에서 **동작 차이 없음(허용)** 으로 확인 |
| 대안 | 진행률을 실제로 쓰게 되면 인터페이스에 콜백을 추가해야 한다. 지금은 추가하지 않았다 |

**도달 불가 분기** (문서 #3) — 일괄 블록의 이마트(원본 3057) · 홈플러스(원본 3059) 분기는 그 두 타입이 건별로 빠져 도달할 수 없다. **Activity 원본에서는 지우지 않았고**, 3 · 4 · 5 타입 파일에서는 자연히 빠졌다.
...
|---|---|---|
| 빌드 | — | `BUILD SUCCESSFUL` |
| 원본 본문 대조 (6종) | 추가 0줄 / 삭제는 분기 골격만 | **추가 0줄**, 삭제 9~14줄(searchType 분기 · publishProgress) |
| 패킷 필드 순서 · 구분자 | 원본 동일 | **동일** (`::` 14필드, 10번째 `Common.selectCompanyCode`, 끝 `GI_L_ID`) |
| 일괄 `packet == ""` → `"af"` | 유지 | **유지** (`==` 비교까지 원본 그대로) |
| 반환 지점 (`s` · `f` · `ss` · `af`) | 원본 동일 | **동일** |
| 스텁(`UnsupportedOperationException`) 잔존 | 0 | **0** (6개 파일 전부 구현 완료) |
| Activity 변경 | 위임 5줄 | **위임 5줄**, 기존 본문 보존 |
| 생산(1)·생산라벨(7) | 무영향 | **무영향** (`shipmentType == null`) |

**주석 라인 번호 정정** — Step 10 주석에 적었던 `원본 N` 값 일부가 추정이었다. Step 0 직전 파일과 내용 대조로 확인해 고쳤다.

| 구분 | 수정 전(추정) | 수정 후(확인) |
|---|---|---|
| 건별 디비접속 분기 | 3005~3013 | **2966~2974** |
| 건별 `publishProgress` | 3025 | **2986** |
| 일괄 비정량 전송 | 3103~3104 | **3064~3065** |
| 일괄 도매 전송 | 3106~3108 | **3067~3069** |
| 일괄 `publishProgress` | 3129 | **3090** |
| 도달 불가 분기 | 3096 · 3098 | **3057 · 3059** (문서 표와 일치) |
...
- [x] Part 3: 변환 수행
- [x] Part 4: 컴파일 확인 — `gradlew assembleDebug` → `BUILD SUCCESSFUL` (2026-09-17)
- [ ] Part 5: 단위테스트 — 실기기 확인 필요 (타입별 수기 중량 입력 → 소비기한 창 또는 바로 저장)
- [ ] Part 6: 회귀테스트 — 실기기 확인 필요 (킬코이·미트센터 상품 수기 입력, 생산(1) 수기 입력 무영향)

**Part 6. 변경 내용** (완료):
- **무엇을**: `inputBtnListener` 의 `work_flag == 0`(수기 입력) 분기 본문을 타입 파일 6개의 `onManualInput()` 으로 이관. `ShipmentType` 에 메서드를 **이 Step에서** 추가했다(§3 "미리 선언하지 않는다")
- **왜**: 중량 절사(0만 다름)와 소비기한 창(0은 센터 조건부, 6은 항상)이 한 리스너에 섞여 있었다
- **어떻게**: 입력값 검증(원본 647~655)은 Activity에 남기고, 통과 후 `shipmentType != null` 이면 `onManualInput()` 으로 위임한다. 기존 본문은 생산(1)·생산라벨(7)용으로 **삭제하지 않았다**

**타입별 접기 결과**

| 타입 | 중량(원본 668) | 바깥 `else if`(원본 711)의 `\|\| LOTTE` | 안쪽 소비기한 창(원본 712) |
|---|---|---|---|
| 이마트(0) | **소수 1자리 절사** | 거짓 → 그 항만 제거 | **참** → 조건문만 제거, 안쪽 else 삭제 |
| 홈플러스(2) · 도매(3) · 이마트비정량(4) · 홈플비정량(5) | 그대로 입력 | 거짓 → 그 항만 제거 | 거짓 → `wet_data_insert` 경로만 |
| 롯데(6) | 그대로 입력 | **참** → `}else{` 로 접고 바깥 else(733~735) 삭제 | **참** → 조건문만 제거, 안쪽 else 삭제 |

> **두 조건을 함께 접지 않았다** (Part 2 지시). 711은 바깥 진입 조건, 712는 안쪽 조건이라 롯데에서도 각각 따로 판정했다.

**접지 않은 것**

| 항목 | 이유 |
|---|---|
| `CENTERNAME` TRD/WET/E·T 판정 | searchType 게이트가 아니라 **데이터 의존**이다. 6종 전부 원문 유지(롯데는 진입 조건이 항상 참이라 조건 자체가 사라진다) |
| 킬코이(`PACKER_CODE`) · 미트센터(`STORE_CODE`) 분기(원본 691~710) | searchType 게이트가 없다. 6종 전부 그대로 유지(문서 #3) |

**저장 경로** — 타입 파일의 `a.wet_data_insert(...)` 는 Activity 위임(Step 8)을 거쳐 **자기 타입의 `onWeightConfirmed`** 로 이어진다. 원본과 같은 경로다.
...
|---|---|---|
| 빌드 | — | `BUILD SUCCESSFUL` |
| 원본 본문 대조 (6종) | 추가 0줄 / 삭제는 접은 조건만 | **추가 0줄**, 삭제 10~25줄(타입별 미해당 경로) |
| Intent extra 키 4종 · `GET_DATA_REQUEST` | 원본 동일 | **동일** |
| `CENTERNAME` 판정 | 5종 유지 | **유지** (롯데는 진입 조건이 항상 참) |
| 킬코이 · 미트센터 분기 | 6종 유지 | **유지** |
| 인터페이스 | `onManualInput()` 1개 추가 | **1개 추가** (총 7개 메서드) |
| Activity 변경 | 위임 6줄 | **위임 6줄**, 기존 본문 보존 |
| 생산(1)·생산라벨(7) | 무영향 | **무영향** (`shipmentType == null`) |
...
**체크리스트**
- [x] Part 1: 분석 완료 확인
- [x] Part 2: 변환 계획 확인 — diff → 목록 작성 → **사용자 확인 완료(2026-09-17)**
- [x] Part 3: 변환 수행 — **이동 0건**(아래 판정 참조). `common/` 을 만들지 않았다
- [x] Part 4: 컴파일 확인 — 코드 변경이 없어 Step 11 빌드 상태 그대로(`BUILD SUCCESSFUL`)
- [x] Part 5: 단위테스트 — **해당 없음** (코드 변경 0건)
- [x] Part 6: 회귀테스트 — **해당 없음** (코드 변경 0건)

**Part 6. 변경 내용** (완료):
- **무엇을**: 타입 파일 6개를 기계적으로 diff해 공통 블록을 전수 조사했다. **코드는 바꾸지 않았다**
- **왜**: Part 1의 기준 3개를 모두 통과하는 대상이 없었다. 기준을 어기면서까지 묶으면 이 문서가 A/B 단계를 나눈 이유가 사라진다
- **어떻게**: 메서드 단위 + 연속 라인 단위(4줄 이상)로 6종 전부 일치하는 구간을 뽑고, 각 구간이 상태를 바꾸는지(`a.필드 =`, `setText`, `DBHandler`, `startActivityForResult` 등) 판정했다

**diff 결과 — 메서드 단위**

| 메서드 | 6종 일치 여부 | 판정 |
|---|---|---|
| `findPackerProduct` | **6종 완전 동일** (30줄) | ✗ callee `find_work_info` 가 이마트비정량(4)만 다르고, `edit_product_name` 을 읽는다. 옮기면 타입으로 되돌아오는 호출이 필요하다 |
| `find_work_info_barcodeGoods` | **6종 완전 동일** (42줄) | ✗ **기준 #3 위반** — 상태 변경 8곳(`work_item_bi_info`, `edit_product_name`, `edit_product_code`, `work_item_barcodegoods`) |
| `find_work_info` | 5종 동일 / 이마트비정량(4) 다름 | ✗ 기준 #2 |
| `onBarcodeScanned` | 3종 / 2종 / 1종 | ✗ 기준 #2 |
| `onWeightConfirmed` | **6종 전부 다름** | ✗ |
| `reprintLabel` | 3종 / 2종 / 1종 | ✗ 기준 #2 |
| `onShipmentLoaded` | 5종 동일 / 롯데(6) 다름 | ✗ 기준 #2 |
| `send` | 3종 / 2종 / 1종 | ✗ 기준 #2 |
| `onManualInput` | 4종 / 1종 / 1종 | ✗ 기준 #2 |

**diff 결과 — 연속 라인 단위**

6종 전부에 존재하는 4줄 이상 구간은 **17블록 556줄**이다. 그중 **순수 계산은 패킷 앞 13필드 조립 하나뿐**이고, 나머지는 전부 Activity 상태·UI·DB·프린터를 건드린다.

| 블록 | 규모 | 상태 변경 | 판정 |
|---|---|---|---|
| 패킷 앞 13필드 조립 (`send`) | 13줄 | 없음 | 기준 3개 통과하나 **이동하지 않음**(아래 사유) |
| 중복검사 · 계근 저장 · 화면 갱신 · 라벨 · 전송 후처리 등 16블록 | 543줄 | 있음 | ✗ 기준 #3 |

**패킷 13줄을 옮기지 않은 사유** (사용자 확인 2026-09-17)

| 항목 | 내용 |
|---|---|
| 이득 | 13줄 × 6 = 78줄 중복 제거 |
| 손실 | 패킷 조립이 두 파일로 쪼개진다. 건별(0·2·6)은 `…GI_L_ID` 로, 일괄(3·4·5)은 `…BOX_ORDER::GI_L_ID##` 로 끝나 **뒷부분은 어차피 타입별**이다 |
| 위험 | 마트사별 전송 필드가 추가되면 갈린다. 기준 #2가 경계하는 "나중에 갈릴 수 있다" 에 그대로 해당한다 |

**결론** — `common/` 폴더를 만들지 않는다. §3 구조도의 `common/` 은 **생성되지 않은 상태로 남는다.**
타입 파일 간 중복은 존재하지만, 이 문서가 §5에서 정한 기준(판단으로 묶지 않고 diff 결과만)을 지킨 결과다.
나중에 6종이 실제로 수렴하면 그때 다시 diff해서 판단한다.
...
- [x] Part 1: 분석 완료 확인
- [x] Part 2: 변환 계획 확인
- [x] Part 3: 변환 수행 — 3곳(Javadoc · 접근제한자 1개 · 주석 1개)
- [x] Part 4: 컴파일 확인 — `gradlew assembleDebug` → `BUILD SUCCESSFUL` (2026-09-17)
- [ ] Part 5: 단위테스트 — **해당 없음에 가깝다**(로직 변경 0건). Step 14 통합 테스트로 확인한다
- [ ] Part 6: 회귀테스트 — 같음

**Part 6. 변경 내용** (완료):
- **무엇을**: 클래스 Javadoc 의 searchType 표를 실제와 맞추고, 타입 파일이 쓰지 않는 공개 필드 1개를 되돌렸다. **로직은 한 줄도 바꾸지 않았다**
- **왜**: Step 2~11에서 필요할 때마다 필드를 열었고, 클래스 주석은 개발66 이전 상태 그대로였다
- **어떻게**: 타입 파일 6개에서 `a.<필드>` 사용 횟수를 전수 조사해 실제로 쓰지 않는 것만 되돌렸다. 생산 경로(§5.1)는 손대지 않았다

**#1 접근제한자 — 되돌린 것 1개**

| 필드 | 연 Step | 타입 파일 사용 | 조치 |
|---|:-:|:-:|---|
| `select_position` | 8 | **0회** | `private` 로 되돌림. Step 11 이후 `reprintLabel(..., int select_position)` **인자**로만 전달된다 |

나머지 26개는 6종 전부가 실제로 쓰고 있어 공개를 유지했다.
사용량이 큰 것부터 `arSM`(379) · `current_work_position`(373) · `work_item_bi_info`(331) · `work_item_fullbarcode`(144) 순이다.
`edit_barcode` 에 남아 있던 "(Step 13에서 재검토)" 표시는 재검토 결과("6종 전부 사용 중이라 유지")로 바꿨다.

**#2 빈 위임 메서드 — 없음**

`show_wetFinishDialog` · `set_scanFlag` · `startShipSelect` 는 전부 실제 로직이 있고 생산·타입 양쪽에서 호출한다. 삭제 대상이 없었다.

**#3 Javadoc searchType 표 정정**

| searchType | 수정 전 | 수정 후 |
|:-:|---|---|
| 0 | 이마트 출하 | (동일) |
| 1 | 생산 투입 | 생산 계근(이노이천) |
| **2** | **누락** | **홈플러스 출하** |
| 3 | 도매 출하 | (동일) |
| **4** | 홈플러스 출하 | **비정량 출하** — 라벨·전송은 이마트 계열 |
| **5** | 롯데 출하 | **홈플러스 비정량 출하** |
| **6** | 원앤원 출하 | **롯데 출하** |
| **7** | **누락** | **생산 라벨 — 미사용**(2026-08-04 제외 결정, 화면 숨김) |

`"4"` 는 상수 주석이 "도매 비정량"이지만 실제 라벨은 `setPrinting`(이마트)이고 로그도 "이마트(비정량)"이다. Javadoc 에 이 불일치를 명시했다.
아울러 **"searchType별 흐름 위치(개발66)"** 단락을 추가해, 비생산 6종은 `ShipmentType` 구현체가 담당하고 생산(1)·생산라벨(7)은 `shipmentType` 이 `null` 이라 이 클래스의 기존 본문을 탄다는 것을 적었다.

**생산 경로(§5.1) 확인**

`setBarcodeMsgProduction` 이 호출하는 `find_PackerProduct` · `find_PackerProductBarcodeGoods` · `wet_data_insert` · `ProgressDlgShipSelect` · `show_wetFinishDialog` · `set_scanFlag` 가 모두 Activity 에 그대로 있고, 생산 진입 분기와 `shipmentType != null` 게이트 5곳도 유지된다.

**미조치로 남긴 것**

| 항목 | 내용 |
|---|---|
| 클래스 Javadoc 의 "Woosim 프린터" 표기 | 이 프로젝트는 Bixolon 전환이라 사실과 다르다. 문서 #3 이 범위를 **searchType 표**로 한정해 이번에는 손대지 않았다. 별건으로 처리한다 |
...
| 3 | 바코드 스캔 — 롯데(6) | ✅ 완료 (2026-09-16, 원본 대조 통과 · WholesaleType 일치 · 빌드 통과) |
| 4 | 바코드 스캔 — 홈플러스(2) | ✅ 완료 (2026-09-16, 원본 대조 통과 · 빌드 통과 · 실기기는 데이터 확보 후) |
| 5 | 바코드 스캔 — 홈플러스비정량(5) | ✅ 완료 (2026-09-16, 중복확인 우회 보존 검증 · 빌드 통과) |
| 6 | 바코드 스캔 — 이마트비정량(4) | ✅ 완료 (2026-09-16, EMART 조건 3건 접힘 검증 · 빌드 통과) |
| 7 | 바코드 스캔 — 이마트(0) + 과도기 분기 제거 | ✅ 완료 (2026-09-17, 원본 대조 통과 · 과도기 분기 제거 · 빌드 통과 · 생산라벨(7)은 진입 경로 없음으로 종결) |
| 8 | 계근 저장 + 라벨 (6종) | ✅ 완료 (2026-09-17, 추가 로직 0줄 · 롯데 박스순번 유지 · 생산(1)·생산라벨(7) 무영향 · 빌드 통과) |
| 9 | 상품 매칭 (6종) | ✅ 완료 (2026-09-17, 전부 매칭 4번만 유지 · 구간추출 차이 보존 · 추가 로직 0줄 · 빌드 통과) |
| 10 | 전송 (6종) | ✅ 완료 (2026-09-17, 건별 3종 · 일괄 3종 · 패킷 동일 · 추가 로직 0줄 · 빌드 통과 · 실전송 테스트 대기) |
| 11 | 수기 입력 (6종) | ✅ 완료 (2026-09-17, 711·712 개별 접기 · CENTERNAME/킬코이 보존 · 추가 로직 0줄 · 빌드 통과) |
| 12 | 공통 추출 (diff) | ✅ 완료 (2026-09-17, 전수 diff 후 **이동 0건** · `common/` 미생성 · 사용자 확인) |
| 13 | Activity 정리 | ✅ 완료 (2026-09-17, Javadoc 표 정정 · `select_position` private 복귀 · 로직 변경 0건 · 빌드 통과) |
| 14 | 통합 테스트 | ⏳ 대기 |
