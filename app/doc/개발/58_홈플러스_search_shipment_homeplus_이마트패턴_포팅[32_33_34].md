...
### 2026-07-23 — 오류 33·34 실제 채택안 변경 (STORE_IN_DATE = H.출고일자)

**아래 4.2/4.3·Step 2/3의 "OUTER APPLY + CONVERT (SD.납기일자 유지)" 계획은 구현 단계에서 다음 방식으로 대체되었다.**

- **실제 채택안**: `search_shipment_homeplus.jsp:59`의 STORE_IN_DATE 출처를 `SD.납기일자` → **`H.출고일자`** 로 변경하고, 불필요해진 `SM_수주상세 SD` LEFT JOIN(99~100줄)을 **완전 제거**.
- **효과**: 오류 33(SM_수주상세 1:N 팽창)은 JOIN 제거로 원천 해소, 오류 34(datetime CONVERT)는 `H.출고일자`가 이미 VARCHAR(8) YYYYMMDD이므로 CONVERT 자체가 불필요한 구조로 전환되어 문제 소멸.
- **근거**: 원본 오라클 두 뷰(VW_PDA_WID_LIST/VW_PDA_WID_HOMEPLUS_LIST) 모두 STORE_IN_DATE=`EOI.STORE_IN_DATE`로 동일 정의이며, 이마트 MSSQL 전환이 이를 `H.출고일자`로 매핑한 채 완료됨. 홈플러스도 **이마트 기준과 일관**되게 `H.출고일자`로 통일(2026-07-23 사용자 결정). original-comparator가 지적한 "SD.납기일자→H.출고일자" 차이는 이마트에도 동일하게 존재하는 기수용 편차로, 이 결정으로 해소.
- **검증**: code-verifier 통과(SELECT 컬럼 24개·index 0~23 불변, 라벨 substring 파싱 호환). 상세: 오류 33·34 문서 상태 참조.
- **미해당**: 오류 32(V뷰 년월조건, 4.1/Step 1)는 본 개정과 무관하며 별개로 남아 있다(미착수).

> 아래 4.2/4.3, Step 2/3 본문은 최초 계획 기록으로 보존한다. 실제 코드는 위 개정안 기준이다.
