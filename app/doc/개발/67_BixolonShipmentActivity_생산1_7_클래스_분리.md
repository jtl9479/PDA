...
- [x] Part 3: 변환 수행
- [x] Part 4: 컴파일 확인 — `gradlew assembleDebug` → `BUILD SUCCESSFUL` (2026-09-17)
- [ ] Part 5: 단위테스트 — Step 8 이후 실기기 확인 필요 (생산 수기 중량 입력)
- [ ] Part 6: 회귀테스트 — Step 8 이후 실기기 확인 필요 (킬코이·미트센터 상품 수기 입력, 비생산 6종 무영향)

**Part 6. 변경 내용** (완료):
- **무엇을**: `inputBtnListener` 의 `work_flag == 0` 분기 본문을 생산 2종의 `onManualInput()` 으로 이관. **이관 작업의 마지막 Step 이다**
- **왜**: 수기 입력 축까지 타입 파일이 소유하면 생산 2종의 `ShipmentType` 7개 메서드가 모두 채워진다
- **어떻게**: 생산 2종은 접기 판정이 도매(3)와 같아 검증된 `WholesaleType.onManualInput` 을 기준으로 삼고 주석만 바꿨다. 입력값 검증(원본 655~663)은 Activity 에 남는다

**타입별 접기 결과** (생산 2종 동일)

| 항목 | 원본 | 처리 |
|---|---|---|
| 중량 처리 | 683 | else 경로(`Double.toString`). EMART 의 `Math.floor(*10)/10` + `%.1f` 블록 삭제 |
| 바깥 `else if` 의 `\|\| LOTTE` | 726 | 거짓 → **그 항만 제거**. CENTERNAME(TRD/WET/E·T) 판정은 데이터 의존이라 **유지** |
| 안쪽 소비기한 창 | 727 | 거짓 → else(원본 746) 경로인 `wet_data_insert` 만 남김 |
| 킬코이 · 미트센터 분기 | 706~725 | searchType 게이트가 없어 **그대로 유지** |

> 726(바깥 진입 조건)과 727(안쪽 조건)을 **함께 접지 않았다**(Part 2 지시, 개발66 Step 11 원칙 계승).
...
|---|---|---|
| 빌드 | — | `BUILD SUCCESSFUL` |
| 원본 대조 (2종) | 접은 조건만 차이 | **삭제 25줄**(EMART 절사 + 소비기한 창), 실질 추가 0줄 |
| 문자열 리터럴 오염 | 0건 | **0건** |
| 생산 2종 상호 비교 | 동일 | **완전 일치** |
| **스텁 잔존** | **0건** | **0건** — 7개 메서드 전부 구현 완료 |
| Javadoc 배치 | 밀림 없음 | **8개 파일 전부 연속 Javadoc 없음** (Step 6 재발 방지책 적용 확인) |
| 런타임 영향 | 0건 | **0건** (Activity 미변경) |
...
| 축 | Step | 생산(1) | 생산라벨(7) |
|---|:-:|---|---|
| 바코드 스캔 | 2 · 3 | `setBarcodeMsgProduction` 본문(S·J 2종) | 공용 `setBarcodeMsg` 복원(W/HW·S·J·B 5종 + 킬코이·센터명) |
| 계근 저장 | 4 | 일반 INSERT · 3자리 | 동일 |
| 계근 라벨 | 4 | **없음** | `setPrinting_prod` |
| 재출력 라벨 | 4 | else(이마트 수기) | `setPrinting_prod` |
| 조회 후처리 | 4 | no-op | no-op |
| 상품 매칭 | 5 | 동일(사본 2개 포함) | 동일 |
| 전송 | 6 | 일괄 · production JSP | 동일 |
| 수기 입력 | 7 | 동일 | 동일 |

**다음은 Step 8 컷오버다.** 지금까지 Step 1~7 은 전부 런타임 영향이 0이었다
(`onCreate` 가 생산을 제외해 두 타입이 생성되지 않았다). Step 8 에서 처음으로 실제 동작이 바뀐다.
