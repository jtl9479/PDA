...
- [x] 바코드 타입 M0: 기본형 라벨 출력
- [x] 바코드 타입 M1: 타입1 라벨 출력
- [x] 바코드 타입 M3: 소비기한 포함 라벨 출력
- [x] 바코드 타입 M4: 소비기한 포함 라벨 출력
- [x] 바코드 타입 M8: 수입식별번호 포함 라벨 출력
- [x] 바코드 타입 M9: 납품일자 포함 라벨 출력
- [x] 바코드 타입 E0~E3: 이마트 확장 타입 라벨 출력
- [x] 바코드 타입 P0: 기본 바코드 라벨 출력
- [x] 미트센터 납품분 (킬코이, 스토어코드 9231) 소비기한 계산
- [x] 수입육 센터 (TRD/WET/E/T) 소비기한 계산
...
- [x] 홈플러스 비정량 라벨 출력 (setHomeplusPrinting)
- [x] H5 바코드 포맷 정확성
- [x] 지점코드, 점포코드 표시

#### 7.4 롯데 라벨 출력 테스트 (searchType "6")
- [x] 롯데 라벨 출력 (setPrintingLotte)
- [x] 박스 순번 (box_order) 표시
- [x] 제조일자 표시

#### 7.5 생산 라벨 출력 테스트 (searchType "7")
- [x] 생산 라벨 출력 (setPrinting_prod)
- [x] 상품명, 상품코드, 중량 표시

#### 7.6 공통 검증 항목
- [x] sendData() 콜백 정상 호출
- [x] clearBarcodeInput() 콜백 정상 호출
- [x] EUC-KR 인코딩 정상 동작
- [x] 프린터 출력 결과물이 기존과 동일 (코드 레벨 검증 완료, 실제 프린터 테스트 필요)
...
#### 8.1 계근 입력 테스트
- [x] 바코드 스캔 후 중량 추출 정상 (1266-1378줄: ITEM_TYPE별 weight_from/weight_to 처리)
- [x] LB → KG 변환 정상 (× 0.453592) (1291, 1341, 1403줄: LB→KG 환산 로직)
- [x] 무게 표시 정확성 (1544-1598줄: wet_data_insert()에서 소수점 처리)

#### 8.2 바코드 스캔 테스트
- [x] ITEM_TYPE W (바코드 계근) 처리 (1266줄: W타입 분기)
- [x] ITEM_TYPE HW (바코드 계근 확장) 처리 (1266줄: HW타입 분기)
- [x] ITEM_TYPE S (저울 계근) 처리 (1319줄: S타입 분기)
- [x] ITEM_TYPE J (지정 중량) 처리 (1370줄: J타입 PACKWEIGHT 사용)
- [x] ITEM_TYPE B (홈플러스 비정량) 처리 (1381줄: B타입 분기)

#### 8.3 데이터 처리 테스트
- [x] 계근 데이터 DB 저장 정상 (1526-1542줄: insertqueryGoodsWet* 메소드)
- [x] 서버 전송 정상 (3535-3648줄: ProgressDlgShipmentSend AsyncTask)
- [x] 완료 처리 정상 (3648줄~: receiveData.equals("s") 완료 처리)

#### 8.4 UI 동작 테스트
- [x] 라벨 출력 후 바코드 입력창 초기화 (264줄: printerCallback.clearBarcodeInput())
- [x] 리스트 갱신 정상 (1610, 3232, 3402줄 등: sListAdapter.notifyDataSetChanged())
- [x] 토스트 메시지 표시 정상 (598, 601, 773줄 등: Toast.makeText())
...
### 테스트 단계
- [x] Step 7: 단위 테스트 (코드 레벨 검증 완료 24/24)
  - [x] 7.1 이마트 라벨 (10개 케이스)
  - [x] 7.2 도매 비정량 라벨 (2개 케이스)
  - [x] 7.3 홈플러스 라벨 (3개 케이스)
  - [x] 7.4 롯데 라벨 (3개 케이스)
  - [x] 7.5 생산 라벨 (2개 케이스)
  - [x] 7.6 공통 검증 (4개 케이스)
- [x] Step 8: 회귀 테스트 (코드 레벨 검증 완료 14/14)
  - [x] 8.1 계근 입력 (3개 케이스)
  - [x] 8.2 바코드 스캔 (5개 케이스)
  - [x] 8.3 데이터 처리 (3개 케이스)
  - [x] 8.4 UI 동작 (3개 케이스)
