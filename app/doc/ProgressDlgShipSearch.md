...
### 개요
ProgressDlgShipSearch.java에서 미사용 변수와 빈 코드 블록을 삭제하였습니다.

### 1. 삭제된 미사용 변수

| 변수명 | 타입 | 원래 위치 | 삭제 사유 |
|--------|------|-----------|-----------|
| `hTemp` | `HashMap<String, String>` | 230행 | 선언만 되고 전혀 사용되지 않음 |

### 2. 삭제된 빈 코드 블록

| 원래 위치 | 삭제된 코드 | 설명 |
|-----------|-------------|------|
| 97-98행 | `if (Common.D) { }` | 내용이 없는 빈 if 블록 |
...
- 모든 설명 주석(Javadoc, 인라인 주석)은 유지되었습니다.
- 이 파일은 원래 잘 문서화되어 있었으며, 정리 대상이 매우 적었습니다.
