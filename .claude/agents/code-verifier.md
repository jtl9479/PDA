...
코드 수정 후 실기기 테스트 전에 전체 흐름을 시뮬레이션한다.

### 시뮬레이션 Phase

| Phase | 내용 | 검증 대상 |
|:-----:|------|----------|
| 1 | PDA → JSP 호출 | WHERE 조건 조립, URL, searchType 분기 |
| 2 | JSP 쿼리 실행 → 응답 | SELECT 컬럼, JOIN, WHERE, out.println 순서 |
| 3 | PDA 파싱 | temp[] 매핑, Shipments_Info setter |
| 4 | 로컬DB 저장 → 화면 표시 | INSERT 컬럼, CREATE TABLE 일치 |
| 5 | 바코드 스캔 → 계근 → 라벨 출력 | searchType별 분기, 차이 컬럼 영향 |
| 6 | 회귀 테스트 | 다른 searchType JSP/Java 미수정 확인 |

### 시뮬레이션 실행 방법
1. 실제 소스 파일을 Read/Grep으로 읽어서 코드 추적
2. 가상 데이터 1행을 생성하여 전체 흐름 추적
3. 각 Phase별 PASS/FAIL 판정
4. 차이 컬럼이 로직(킬코이 분기, 계근 방식, 바코드 매칭, 서버 전송)에 미치는 영향 분석
...
| Phase | 검증 항목 | 결과 | 비고 |
|:-----:|----------|:----:|------|
| 1 | PDA → JSP 호출 | PASS/FAIL | 상세 |
| 2 | JSP 쿼리 실행 | PASS/FAIL | 상세 |
| ...
...
## 주의사항
- 검증/시뮬레이션만 수행하고 코드를 직접 수정하지 않을 것
...
- 시뮬레이션 시 실제 소스 파일을 반드시 읽어서 검증 (문서만 보고 판단 금지)
